...
namespace sepia {
    ...
    class handle_byte<type::generic> {
        public:
        handle_byte() : _state(state::idle), _index(0), _bytes_size(0), _offset_and_previous_t(0, 0) {}
        ...
        virtual bool operator()(uint8_t byte, generic_event& generic_event) {
            ++_offset_and_previous_t.first;
            switch (_state) {
                case state::idle:
                    _offset_and_previous_t.first = 1;
                    _offset_and_previous_t.second = generic_event.t;
                    if (byte == 0b11111111) {
        ...
        std::pair<std::size_t, uint64_t> _offset_and_previous_t;
    };
    ...
    class handle_byte<type::dvs> {
        ...
        handle_byte(uint16_t width, uint16_t height) :
            _width(width), _height(height), _state(state::idle), _offset_and_previous_t(0, 0) {}
        handle_byte(const handle_byte&) = default;
        handle_byte(handle_byte&&) = default;
        ...
        virtual bool operator()(uint8_t byte, dvs_event& dvs_event) {
            ...
            switch (_state) {
                case state::idle:
                    _offset_and_previous_t.first = 1;
                    _offset_and_previous_t.second = dvs_event.t;
                    if (byte == 0b11111111) {
    ...
    class handle_byte<type::atis> {
        ...
        virtual bool operator()(uint8_t byte, atis_event& atis_event) {
            ...
            switch (_state) {
                case state::idle:
                    _offset_and_previous_t.first = 1;
                    _offset_and_previous_t.second = atis_event.t;
    ...
    class handle_byte<type::color> {
        ...
        virtual bool operator()(uint8_t byte, color_event& color_event) {
            ...
            switch (_state) {
                case state::idle: {
                    _offset_and_previous_t.first = 1;
                    _offset_and_previous_t.second = color_event.t;
                    if (byte == 0b11111111) {
    ...
    class write_to_reference<type::generic> {
        ...
        /// flush synchronizes with the undeerlying storage device.
        virtual void flush() {
            _event_stream.flush();
        }
    ...
    /// base_observable is a common base type for observables.
    class base_observable {
        ...
        base_observable& operator=(const base_observable&) = default;
        base_observable& operator=(base_observable&&) = default;
    ...
    class observable : public base_observable {
        public:
        observable(
                        ...
                        case dispatch::synchronously_but_skip_offset: {
                            ...
                            while (_running.load(std::memory_order_relaxed)) {
                                ...
                                if (_event_stream->eof()) {
                                    ...
                                    auto offset_and_previous_t = handle_byte.offset_and_previous_t();
                                    offset_and_previous_t.first =
                                        static_cast<std::size_t>(static_cast<std::streamoff>(_event_stream->tellg()))
                                        - offset_and_previous_t.first;
                                    _after_buffer(offset_and_previous_t);
                                    if (_must_restart()) {
                                        _event_stream->clear();
    ...
    inline std::unique_ptr<observable<event_stream_type, HandleEvent, AfterBuffer, HandleException, MustRestart>>
    make_observable(
        std::unique_ptr<std::istream> event_stream,
        HandleEvent&& handle_event,
        AfterBuffer&& after_buffer,
        ...
        return sepia::make_unique<
            observable<event_stream_type, HandleEvent, AfterBuffer, HandleException, MustRestart>>(
    ...
    template <type event_stream_type, typename HandleEvent, typename AfterBuffer>

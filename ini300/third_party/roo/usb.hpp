    ...
    namespace usb {
        ...
        class interface {
            ...
            interface(std::shared_ptr<libusb_context> context, libusb_device* device) :
                _context(std::move(context)), _device(device) {
                ...
                // throw_on_error("resetting the device",
                // libusb_reset_device(_handle.get())); @DEV hangs
            }
            ...
            virtual void control_transfer(
                ...
                if (bytes_transferred != buffer.size()) {
                    throw std::runtime_error(message + " failed: non-matching data and transfer sizes");
                }
            }
